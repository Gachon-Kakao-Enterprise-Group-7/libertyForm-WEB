...
function Groupcontrol(props) {
  ...
  let options = [
    ...new Set(state && state.map((contact) => contact.relationship)),
  ]; // 사용자의 연락처에서 릴레이션쉽으로 옵션 배열을 만들어 준다
  console.log(options);

  const addAlluser = (e) => {
    const temp = state
      .filter((contact) => contact.relationship === groupValue)
    ...
    <div>
      ...
      <GroupControll>
        <Autocomplete // 그룹 선택하는 콤보 박스
          ...
          sx={{ width: "50%", m: 3 }}
          renderInput={(params) => (
            <TextField {...params} label="그룹을 선택하세요" />
          )}
        />
        {groupValue !== null && (
          <AddBtn onClick={addAlluser}>
            <UserAddSvgW />
            <p>전체 추가</p>
          </AddBtn>
        )}
            ...
            {state
              ...
              .map((contact, index) => (
                <TableBody key={index}>
                  ...
                  <TableCell align="center" padding="none">
                    {contact.member && <Check width="20px" />}
                  </TableCell>
