...
function EditSurvey() {
  ...
  useEffect(() => {
    // 처음에 서버에서 받아오는 설문 상세 정보!
    setLoading(true);
    const jwt = localStorage.getItem("jwt");
    axios
      .get(`${process.env.REACT_APP_DB_HOST}/survey/${surveyCode}`, {
        headers: {
      ...
      .then((res) => {
        switch (res.data.code) {
          case 1000:
            const choiceQuestions = res.data.result.choiceQuestions.map(
              (
                item // choiceQuestions 데이터 전처리 과정
              ) => ({
                ...item.question,
            ...
            setSurveyDetail((prev) => ({
              ...
              }), // res.data에 있는 객관식과 주관식을 하나의 배열로 합치고, 문제 번호순으로 정렬
            }));
            ...
            setQlength(
              res.data.result.choiceQuestions.length +
                res.data.result.questions.length
            );
            setLoading(false);
            break;
          default:
  ...
  const changeDate = (date) => {
    // 날짜 형식을 백엔드에 보내줘야 할 양식으로 변환하는 함수
        ...
        ...survey
          ...
          .map(
            (
              ...
              choices: item.mcitem.map((mcitem, index) => ({
                name: mcitem.name,
                number: index + 1,
      ...
      questions: [
        ...
        ...survey
          ...
          .map(
            (
              item,
              index // 필터로 객관식 아닌 질문들만 걸러서 questions에 넣어준다.
            ) => ({
              questionTypeId: item.type,
              questionId: item.questionId,
              name: item.q,
  ...
  const onChange = (e) => {
    ...
    setSurvey(
      survey.map((item) =>
        item.id === targetId + 1 ? { ...item, q: q } : item
      )
  ...
  const onChangeDescription = (e) => {
    ...
    setSurvey(
      survey.map((item) =>
        item.id === targetId + 1 ? { ...item, description: description } : item
      )
    ); // 사용자가 값을 입력하게되면 onChange함수 실행되고 setSurvey함수를 통해 survey를 map해서 item의 id와 targetid가 같으면 q를 input태그에 입력한 값으로 한다.
  };
      ...
      setSurvey(
        survey.map((item) =>
          item.id === targetId + 1
            ? {
                ...item,
                mcitem: [...item.mcitem, { name: mcitem, choiceId: -1 }],
              }
            : item
        )
      );
  ...
  const onToggle = (e) => {
    ...
    setSurvey(
      survey.map((item) =>
        item.id === targetId + 1 ? { ...item, required: !item.required } : item
      )
    );
  };
  ...
  const delMcItem = (e) => {
    ...
    setSurvey(
      survey.map((item) =>
        item.id === index + 1 ? { ...item, mcitem: temp } : item
      )
    );
  };
  ...
  const requestSubmit = () => {
    ...
    } else if (survey[0].q.length < 1) {
      // 첫번째 질문의 길이가 0일때(객관식 제외하고 나머지)
      Swal.fire({
        title: "Error!",
    ...
    } else if (survey[0].type === "3" && survey[0].mcitem.length === 0) {
      //객관식일때
      Swal.fire({
        title: "Error!",
  ...
  const sendToServer = async () => {
    ...
    await axios
      .patch(`${process.env.REACT_APP_DB_HOST}/survey/modify`, postData, {
  ...
  return (
    <div>
              ...
              {title.length > 0 ? (
                <PreviewCard>
                  <PreviewText style={{ marginTop: "10px" }}>
                    {title.length > 0 && `${title}에 관한 설문입니다.`}
                  </PreviewText>
      ...
      {survey.map(
        (
          item,
          index // survey의 개수에 따라 ItemDiv를 보여준다.
        ) => (
          <BlockDiv key={index}>
            <ItemDiv>
              <NumberingDiv>
                ...
                >
                  <FormControlLabel
                    ...
                    onClick={(e) => {
                      ...
                      ); // 객관식 버튼을 눌렀을때 setsurvey를 통해 survey의 type을 변경한다
                    }}
                  />
                  <FormControlLabel
                    checked={item.type === 4}
                  ...
                  <FormControlLabel
                    ...
                    onClick={(e) => {
                      setSurvey(
                        survey.map((item) =>
                          item.id === index + 1 ? { ...item, type: 2 } : item
                        )
                      ); // 단답형 버튼을 눌렀을때 setsurvey를 통해 survey의 type을 변경한다
                    }}
                  />
                  <FormControlLabel
                    checked={item.type === 1}
                    ...
                    onClick={(e) => {
                      setSurvey(
                        survey.map((item) =>
                          item.id === index + 1 ? { ...item, type: 1 } : item
                        )
                      ); // 장문형 버튼을 눌렀을때 setsurvey를 통해 survey의 type을 변경한다
                    }}
                  />
                  <FormControlLabel
                    checked={item.type === 5}
                    ...
                    onClick={(e) => {
                      setSurvey(
                        survey.map((item) =>
                          item.id === index + 1 ? { ...item, type: 5 } : item
                        )
                      ); // 감정바 버튼을 눌렀을때 setsurvey를 통해 survey의 type을 변경한다
                    }}
                  />
                  <FormControlLabel
                    checked={item.type === 6}
                    ...
                    onClick={(e) => {
                      setSurvey(
                        survey.map((item) =>
                          item.id === index + 1 ? { ...item, type: 6 } : item
                        )
                      ); // 선형표현 버튼을 눌렀을때 setsurvey를 통해 survey의 type을 변경한다
                    }}
                  />
                </RadioGroup>
              </FormControl>
              ...
              {survey[index].type === "3" && ( // 객관식 단일
                <>
                  ...
                  <input
                    ...
                    placeholder=""
                    onChange={(e) => {
                      setMultiChoiceItem(e.target.value);
                    }}
                  ></input>
                  <McitemAddBtn onClick={addMcItem} data-id={index}>
                    추가
                  </McitemAddBtn>
                  <StyledOl>
                    {survey[index].mcitem.map((mcitem, mcitemIndex) => (
                      <StyledLi
                        value={mcitemIndex}
                        data-id={index}
                        onClick={delMcItem}
              ...
              {survey[index].type === "2" && ( //단답형
                <>
                  ...
                  <input
                    placeholder=""
                    data-id={index}
                    value={survey[index].description}
                    style={{ width: "100%" }}
      ...
      <FuncDiv>
        ...
        <Surveybutton style={{ marginRight: "0.5rem" }} onClick={requestSubmit}>
          설문 수정하기
        </Surveybutton>
        <hr />
        <button
        ...
        >
          JSON타입으로 뽑아내기 <br /> 버튼 클릭 후 콘솔에서 확인하세요.작동안될
          시 모달창 열었다가 닫기
        </button>
      </FuncDiv>
      <Modal
      ...
      >
        <ModalHeader>
          <ModalDelete
            onClick={() => {
              // 모달창을 닫았을때, closeModal을 통해 모달을 닫아주고, PostData 즉, 백엔드로 보낼 데이터도 다시 초기화 해주는 작업 진행
              closeModal();
              setPostData({
                survey: {
                  name: "",
