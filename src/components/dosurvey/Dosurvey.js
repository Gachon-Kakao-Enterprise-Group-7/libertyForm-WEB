...
function Dosurvey() {
  ...
  const [postData, setPostData] = useState({
    // 피설문자의 설문 응답 결과를 서버에 보내기 위해 변환한 형태의 데이터
  ...
  useEffect(() => {
    if (params.surveyCode) {
      ...
      axios
        ...
        .then((res) => {
          console.log(
            "처음에 데이터 불러오고 그다음에는 실행되면 안되는 useEffect"
          );
    ...
    } else if (params.privateSurveyCode) {
      // 사용자 식별 번호가 있는 설문을 실시 할 때
      setLoading(true);
      const jwt = localStorage.getItem("jwt");
      axios
        .get(
          `${process.env.REACT_APP_DB_HOST}/manage/read/${params.privateSurveyCode} `,
          {
            headers: {
              Authorization: "Bearer " + jwt,
            },
  ...
  useDidMountEffect(() => {
    // surveyDetail -> newSurveyDetail (변경내용 : 객관식, 주관식을 합쳐준다.)
    setNewSurveyDetail({
      ...surveyDetail,
      questions: [
        ...
        ...choiceQuestions.map((item) => ({
          ...item.question,
          mcitem: item.choices.map((mcitem) => mcitem.name),
        })),
      ],
    });
  }, [surveyDetail]);
  ...
  useDidMountEffect(() => {
    // newSurveyDetail -> sortedSurveydetail (변경내용 : 합쳐준 설문정보를 number에 따라 정렬한다.)
  ...
  const startSurvey = () => {
    const remainTime =
      new Date(surveyDetail.survey.expirationDate) - new Date() + 54000000; // 54000000 = 15시간
    if (remainTime > 0) {
  ...
  const dataConvert = () => {
    setPostData((prev) => ({
      ...prev,
      surveyId: sortedSurveyDetail.survey.surveyId,
        ...
        case 3: // 객관식(단일)
          setPostData((prev) => ({
            ...
            singleChoiceResponse: [
              ...prev.singleChoiceResponse,
              { questionNumber: index + 1, choiceNumber: result[index] },
            ],
          }));
          break;
        case 4: // 객관식(복수)
          ...
          setPostData((prev) => ({
            ...
            multipleChoiceResponse: [
              ...prev.multipleChoiceResponse,
              { questionNumber: index + 1, choices },
            ],
          }));
          break;
        case 5: // 감정바
    ...
    await axios
      ...
      .then((res) => {
        ...
        switch (res.data.code) {
          case 1000:
            console.log(
              "만약 privateSurvey라면 설문 제출을 했다는 코드를 보내줘야한다."
            );
            if (params.privateSurveyCode) {
              const jwt = localStorage.getItem("jwt");
              axios
                .get(
                  `${process.env.REACT_APP_DB_HOST}/manage/submit/${params.privateSurveyCode}`,
                  {
                    headers: {
                      Authorization: "Bearer " + jwt,
                    },
                ...
                .catch((Error) => {
                  console.log(
                    Error,
                    "privateSurvey코드 백엔드에 전송하는 과정에서 생긴 오류"
                  );
                });
            }
            Swal.fire({
  ...
  const onSubmit = () => {
    if (sortedSurveyDetail.questions[showSurveyNumber - 1].answerRequired) {
      // 필수문항이면
      if (result[showSurveyNumber - 1] != null) {
        // 문항에 응답했으면
  ...
  const nextQuestion = () => {
    if (sortedSurveyDetail.questions[showSurveyNumber - 1].answerRequired) {
      // 필수문항이면
      if (
        showSurveyNumber < sortedSurveyDetail.questions.length &&
        result[showSurveyNumber - 1] !== undefined
      ) {
        //마지막 문항이 아니고 && 설문에 응답했으면
        setShowSurveyNumber(showSurveyNumber + 1); // 다음 문항으로 넘어가줘라
        if (result[showSurveyNumber] === undefined) {
          // 다음 문항에 내용이 아직 없다면
      ...
      if (showSurveyNumber < sortedSurveyDetail.questions.length) {
        //필수 문항이 아님'
        if (result[showSurveyNumber - 1] === undefined) {
          // 문항 답변에 아무것도 없을 경우에
  ...
  const onChangeType4 = (e) => {
    ...
    if (e.target.checked) {
      ...
      console.log(result);
      if (result[showSurveyNumber - 1] === undefined) {
        // 기존 저장된 정보가 없으면 그냥 적는다
        temparr[showSurveyNumber - 1] = [e.target.name];
        setResult(temparr);
        setInputs(temparr[showSurveyNumber - 1]);
      } else {
        // 기존에 저장된 정보가 있으면 불러와서 덮어 씌운다
        temparr[showSurveyNumber - 1] = [
          ...new Set([...result[showSurveyNumber - 1], e.target.name]),
        ];
      ...
      // e.target.checked가 false 즉, 체크박스 선택이 해제 될때 실행된다.
      let temparr = result; // 임시변수에 현재 설문 응답 정보를 가져오고(모든 질문의 응답)
      const filterdtemparr = temparr[showSurveyNumber - 1].filter(
        (choice) => choice !== e.target.name
      ); // 선택을 해제한 choice에 대해서 가지고 있던 result에서 뺀다
  ...
  if (loading) return <>로딩중입니다. 너무 오래걸리면 AXIOS 오류가 있는것</>;
  ...
  return (
    <BackgroundDiv thumbnailImgUrl={sortedSurveyDetail.survey.thumbnailImgUrl}>
      {showSurveyNumber === 0 && ( //설문 시작화면 보여주기
        <StartCard>
          <QuestionTitle style={{ marginTop: "30px" }}>
            {sortedSurveyDetail.survey.name}에 관한 설문입니다.
          </QuestionTitle>
          <QuestionTitle>{sortedSurveyDetail.survey.description}</QuestionTitle>
          <QuestionTitle style={{ marginTop: "30px" }}>
            설문 문항은 총 {sortedSurveyDetail.questions.length}문항 입니다.{" "}
          </QuestionTitle>
            ...
            <ProgressBarDiv>
              <ProgressBar
                completed={Math.round(
                  (result.length / sortedSurveyDetail.questions.length) * 100
                )}
                bgColor="#ff7800"
                labelColor="#f6f6f6"
              />
            ...
            <SurveyCard>
              ...
              {showSurveyNumber === sortedSurveyDetail.questions.length ? ( // 설문의 마지막 문항일때 조건
                <SurveyFooter>
                  ...
                  >
                    <SurveyNextButton onClick={prevQuestion}>
                      이전문항
                    </SurveyNextButton>
                    <SurveyFinalButton onClick={onSubmit}>
                      제출하기
                    </SurveyFinalButton>
                  </div>
                </SurveyFooter>
                ...
                <SurveyFooter>
                  ...
                  >
                    ...
                    <SurveyNextButton onClick={nextQuestion}>
                      다음문항
                    </SurveyNextButton>
                  </div>
                </SurveyFooter>
