...
function Mksurvey() {
  ...
  const [survey, setSurvey] = useState([
    { id: 0, q: "", type: "", required: false },
  ]); // 현재 만들고 있는 survey에 대한 정보를 담고있음
  ...
  const onToggle = (e) => {
    ...
    setSurvey(
      survey.map((item) =>
        item.id === targetId ? { ...item, required: !item.required } : item
      )
    );
  };
  ...
  const delMcItem = (e) => {
    ...
    setSurvey(
      survey.map((item) =>
        item.id === index ? { ...item, mcitem: temp } : item
      )
    );
  };
  ...
  const onLoadQuestionFile = (e) => {
    ...
    switch (fileExtension) {
      case "jpg": //jpg일 경우
        newFile = new File([oldFile], `${questionNumber}.jpg`, {
          type: oldFile.type,
        }); // File생성자를 이용해서 oldFile의 이름을 바꿔준다
        setImgs((prev) => [...prev, newFile]); // 저장
        break;
      case "png":
        newFile = new File([oldFile], `${questionNumber}.png`, {
          type: oldFile.type,
        });
  ...
  const sendToServer = async () => {
    ...
    imgs.forEach((item) => formData.append("questionImgs", item));
    console.log(imgs);
                ...
                >
                  ...
                  <FormControlLabel
                    ...
                    onClick={(e) => {
                      setSurvey(
                        survey.map((item) =>
                          item.id === index ? { ...item, type: "2" } : item
                        )
                  ...
                  <FormControlLabel
                    ...
                    onClick={(e) => {
                      setSurvey(
                        survey.map((item) =>
                          item.id === index ? { ...item, type: "1" } : item
                        )
                  ...
                  <FormControlLabel
                    ...
                    onClick={(e) => {
                      setSurvey(
                        survey.map((item) =>
                          item.id === index ? { ...item, type: "5" } : item
                        )
                  ...
                  <FormControlLabel
                    ...
                    onClick={(e) => {
                      setSurvey(
                        survey.map((item) =>
                          item.id === index ? { ...item, type: "6" } : item
                        )
              ...
              {survey[index].type === "2" && (
                <>
                  ...
                  <input
                    placeholder=""
                    data-id={index}
                    value={survey[index].discription}
                    style={{ width: "100%" }}
      ...
      <FuncDiv>
        <Surveybutton
          ...
          onClick={() => {
            setSurvey([
              ...survey,
              { id: id.current, q: "", type: "", required: false },
            ]);
            id.current += 1;
          }}
        >
