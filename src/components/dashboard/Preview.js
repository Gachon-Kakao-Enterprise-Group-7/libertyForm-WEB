...
function Preview(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_DB_HOST}/survey/${code}`, {
        headers: {
  ...
  useDidMountEffect(() => {
    let choice = surveyDetail.choiceQuestions.map((choiceQuestion) => ({
      ...choiceQuestion.question,
      choices: choiceQuestion.choices,
    })); // 객관식 문제만 따로 빼준다
    setNewSurveyDetail((prev) => ({
      ...surveyDetail,
      questions: [...surveyDetail.questions, ...choice],
    })); // 주관식과 객관식 문제를 합쳐준다.
  }, [surveyDetail]);
  ...
  if (!newSurveyDetail) return null;
  return (
    <div>
      ...
      <PreviewContent>
        설문 설명 : {surveyDetail.survey.description}
      </PreviewContent>
      <PreviewContent>
        설문 기간 : {surveyDetail.survey.createdAt} ~{" "}
        {surveyDetail.survey.expirationDate}
      </PreviewContent>
        ...
        <Table sx={{ minWidth: 350 }} aria-label="simple table">
          ...
          {newSurveyDetail.questions
            ...
            .map(
              (
                question,
                index // 문제 번호에 따라 오름차순으로 정렬 후 map함수 반복 시작
              ) => (
                <TableBody key={index + 1}>
                  ...
                  {question.questionTypeId === 1 && (
                    <TableCell align="center">장문형</TableCell>
                  )}
                  {question.questionTypeId === 2 && (
                    <TableCell align="center">단답형</TableCell>
                  )}
                  {question.questionTypeId === 3 && (
                    <TableCell align="center">객관식(단일)</TableCell>
                  )}
                  {question.questionTypeId === 4 && (
                    <TableCell align="center">객관식(복수)</TableCell>
                  )}
                  {question.questionTypeId === 5 && (
                    <TableCell align="center">감정바</TableCell>
                  )}
                  {question.questionTypeId === 6 && (
                    <TableCell align="center">선형표현</TableCell>
                  )}
                  <TableCell align="center">{question.name}</TableCell>
                  <TableCell align="center">
                    {"choices" in question &&
                      question.choices.map((mcitem) => (
                        <li key={mcitem.number}>{mcitem.name}</li>
                      ))}
                  </TableCell>
                </TableBody>
              )
