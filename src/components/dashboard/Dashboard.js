              ...
              <TasksWrapper>
                ...
                {state && (
                  state.filter((survey, index) => (Math.ceil((new Date(`${survey.expirationDate}:00:00:00`) - now) / (1000 * 60 * 60 * 24))) >= 0).map((survey, index) => (
                    <TCards surveyId={survey.surveyId} code={survey.code} key={index} title={survey.name} expirationDate={survey.expirationDate} createdAt={survey.createdAt} thumbnailImgUrl={survey.thumbnailImgUrl} end={false} />
                  ))
                )}
              </TasksWrapper>
            </TaskWrapper>
          ...
          {ongoingSurvey + expiredSurvey > 0
            ...
            <TaskWrapper>
              ...
              <TasksWrapper>
                {state && (
                  state.filter((survey, index) => (Math.ceil((new Date(`${survey.expirationDate}:00:00:00`) - now) / (1000 * 60 * 60 * 24))) < 0).map((survey, index) => (
                    <TCards surveyId={survey.surveyId} code={survey.code} key={index} title={survey.name} expirationDate={survey.expirationDate} createdAt={survey.createdAt} thumbnailImgUrl={survey.thumbnailImgUrl} end={true} />
                  ))
                )}
              </TasksWrapper>
            </TaskWrapper>
