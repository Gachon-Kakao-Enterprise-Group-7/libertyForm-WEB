  ...
  const onSummit = () => {
    ...
    inputs.password === inputs.checkPassword
      ? //여기 조건이 모두 만족할때, ?뒤에 문장 실행, 하나라도 거짓일 경우 :뒤에 문장 실행
        axios
          .post(`${process.env.REACT_APP_DB_HOST}/members`, inputs)
        ...
        <CardWrapper initial={{ opacity: 0.5 }} animate={{ opacity: 1 }}>
          ...
          <CardBody>
            <CardFieldset>
              ...
              {!regex.test(email) && inputs.email.length > 0 && (
            ...
            <CardFieldset>
              <CardInput
                ...
                required
              />
              {!regPass.test(password) && inputs.password.length > 0 && (
                <span style={{ color: "red" }}>
                  비밀번호는 영문과 특수문자 숫자를 포함하며 8자 이상이어야
                  합니다
                  <br />
                </span>
              )}
            ...
            <CardFieldset style={{ marginTop: "30px", marginBottom: "30px" }}>
              <CardButton type="button" onClick={onSummit}>
                회원가입
              </CardButton>
            </CardFieldset>
            <hr />
